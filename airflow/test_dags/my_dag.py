...
with DAG('my_dag', default_args=default_args, schedule='@daily', catchup=False) as dag:
    t0 = BashOperator(task_id='ls_data', bash_command='echo "Hello! I am task0"', retries=2, retry_delay=timedelta(seconds=15))

    t1 = BashOperator(task_id='download_data',
                      bash_command='echo "Hello! I am task1"',
                      retries=2, retry_delay=timedelta(seconds=15))

    t2 = BashOperator(task_id='check_file_exists', bash_command='echo "Hello! I am task3"',
                      retries=2, retry_delay=timedelta(seconds=15))
